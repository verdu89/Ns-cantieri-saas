...
export default function Orders() {
  ...
  const [depositCollectedAt, setDepositCollectedAt] = useState(todayInput);
  ...
  const handleSave = async () => {
    ...
    if (officeWorkflowEnabled && !editingId) {
      const amount = Number.parseFloat(depositAmount.replace(",", "."));
      if (!Number.isFinite(amount) || amount <= 0) {
        toast.error("Inserisci l'importo acconto incassato ❌");
        return;
      }
      if (!depositCollectedAt) {
        toast.error("Inserisci la data acconto ❌");
        return;
      }
    }
    ...
    try {
      if (editingId) {
        ...
        const depositIso = depositCollectedAt
          ? new Date(`${depositCollectedAt}T12:00:00`).toISOString()
          : undefined;
        ...
        const created = await jobOrderAPI.create({
          ...
          ...(officeWorkflowEnabled
            ? {
                hasMeasurements,
                depositAmount: Number.parseFloat(depositAmount.replace(",", ".")),
                depositCollectedAt: depositIso,
      ...
      setDepositAmount("");
      setDepositCollectedAt(todayInput());
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        description={
          officeWorkflowEnabled
            ? "Elenco completo. Per caricare una commessa in ufficio (con acconto) usa la pagina Ufficio o la scheda cliente."
            : "Ordini di lavoro e cantieri."
        }
        actions={
          <div className="flex flex-wrap gap-2">
            {officeWorkflowEnabled && (
              <Button
                type="button"
                variant="outline"
                onClick={() => navigate("/backoffice/office")}
          ...
          <div className={`${modalPanelClass} max-w-lg space-y-3 p-5 sm:p-6`}>
            ...
            {officeWorkflowEnabled && !editingId && (
              <div className="space-y-3 rounded-xl border border-sky-200 bg-sky-50/40 p-4">
                ...
                <label className="flex items-start gap-2 text-sm">
                  ...
                  <span>
                    ...
                    <span className="mt-0.5 block text-xs font-normal text-sky-800/80">
                      Parte in «Da mandare» con conferma già registrata.
                    </span>
                  </span>
                </label>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <input
                      type="date"
                      value={depositCollectedAt}
                      onChange={(e) => setDepositCollectedAt(e.target.value)}
                      className={inputFieldClass}
                    />
                  </div>
                </div>
                <div>
                  <label className="mb-1 block text-xs font-medium text-slate-700">
                    Data prevista consegna (dall&apos;ordine)
                  </label>
                  <input
                    type="date"
