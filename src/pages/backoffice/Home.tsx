...
export default function Home() {
  ...
  const filtered = useMemo(() => {
    ...
    if (ql) {
      res = res.filter((j) =>
        jobMatchesListSearch(j, getOrder(j), getCustomer(j), ql)
      );
    }
    ...
    return res;
  }, [jobs, status, q, sortOrder, getOrder, getCustomer]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className={`space-y-3 ${filterBarClass}`}>
        ...
        <input
          type="search"
          placeholder="Cerca cliente, commessa, città, indirizzo, telefono, note…"
          value={q}
        ...
        <ListSearchStatus
          ...
          totalCount={searchActive ? jobsAfterStatus.length : filtered.length}
          itemSingular="lavoro"
          itemPlural="lavori"
          isSearchActive={searchActive}
          isNarrowed={searchActive}
        />
      </div>
