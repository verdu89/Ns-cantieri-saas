...
export function navDropdownPanel(theme: "light" | "dark"): string {
  return [
    "absolute left-0 z-50 mt-2 w-56 rounded-xl py-1 shadow-lg ring-1 ring-slate-900/5",
    theme === "light"
...
export function navTopbarClass(theme: "light" | "dark"): string {
  return [
    ...
    // Cap 8 / Android 15+: safe-area status bar (SystemBars --safe-area-inset-* + env())
    "pt-[max(0.75rem,var(--safe-area-inset-top,env(safe-area-inset-top,0px)))]",
    theme === "light"
